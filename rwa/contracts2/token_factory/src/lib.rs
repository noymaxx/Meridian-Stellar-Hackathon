...
pub struct DeployedToken {
    ...
    pub trusted_issuers_registry_address: Address,
    pub deployed_at: u64,
...
impl TokenFactory {
    ...
    pub fn deploy_srwa_token(
        ...
        let deployed_token = DeployedToken {
            ...
            trusted_issuers_registry_address: trusted_issuers_registry_address.clone(),
    ...
    fn generate_address(env: &Env, salt: &BytesN<32>, suffix: &str) -> Address {
        ...
        let mut data = salt.to_array().to_vec();
        data.extend_from_slice(suffix.as_bytes());
        let hash = env.crypto().sha256(&data.into());
        Address::from_string(&String::from_str(env, &format!("C{}", hex::encode(&hash.to_array()[..16]))))
    }
