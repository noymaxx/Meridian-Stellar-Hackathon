...
#[contractimpl]
impl LockupModule {
    pub fn init_lockup(env: Env, admin: Address) {
    ...
    pub fn check_lockup(env: Env, context: TransferContext) -> bool {
        let locked_amount = Self::get_locked_amount(env.clone(), context.token.clone(), context.from.clone());
