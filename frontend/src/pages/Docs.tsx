...
export default function Docs() {
  ...
  const sections = [
    {
      id: "part1-vision",
      title: "Part I: Technical Vision & Strategy",
      icon: BookOpen,
      articles: [
        { 
          title: "1. Our Vision", 
...
- P-1 (Compliance correctness): every state mutation that alters token balances MUST call Compliance.can_transfer()
- P-2 (Determinism): given the same set of claims, modules, and integration allowlists, can_transfer is pure and deterministic
- P-3 (Oracle hygiene): effective price for collateral logic MUST be clamp(TWAP_Reflector, NAV±band) * (1-haircut)
- P-4 (Composable by default): any dApp that can call SEP-41 transfer can support SRWA without bespoke KYC code
- P-5 (Upgrade discipline): upgrades MUST be governed by multisig + timelock with explicit storage migration` 
        },
        { 
          title: "2. Product & Market Thesis", 
          content: `**The Problems:**
- C-1 Fragmented compliance: today, each protocol implements ad-hoc eligibility checks
- C-2 NAV vs on-chain price: RWA tokens need valuation integrity; pure spot is noisy, pure NAV is stale
- C-3 Liquidity silos & UX: institutions want one standard rail to access lending, swaps, vaults
- C-4 Auditability & operations: regulated actors require explicit event trails, force actions, and deterministic policies

**Our Solution:**
- A-1 Compliance on the token (SRWA): identity, claims, sanctions, jurisdictions, accreditation, lockups, transfer windows
- A-2 Hybrid price oracle: Reflector TWAP/FX clamped by custodial NAV (haircut + bands + staleness/degraded mode)
- A-3 Motorized composability: Blend provides isolated lending spokes; SoroSwap provides execution and paths
- A-4 Operational rigor: deterministic TokenFactory, admin console for registries and modules` 
        },
        { 
          title: "3. Value Proposition", 
          content: `**For Issuers/Banks:**
...
- SLA: eligibility update < T+1 block; registry mutations reflected atomically
...
- Analytics: DeFindex + event streams for APY/TVL/borrows/liq slippage` 
        ...
        { 
          title: "4. Design Tenets & Trade-offs", 
          content: `**Token-first compliance (SRWA)** → pro: composability, single source of truth; con: more checks per transfer (mitigated by compact storage & caching)
...
**Upgrade discipline** → pro: safer evolution; con: slower changes (intended for institutional trust)` 
        ...
        { 
          ...
          content: `**Token-first Architecture:**
...
- **Oracle Adapter**: Clamps TWAP with NAV bands and applies haircuts` 
        ...
        { 
          title: "6. Institutional Use-cases", 
          content: `**Treasury Credit backed by T-Bills (SRWA-TBill → USDC)**
...
- Larger lot caps on liquidation; stronger backstop requirement` 
    ...
    {
      id: "part2-srwa-spec",
      title: "Part II: SRWA Token & Compliance Specification",
      icon: Shield,
      articles: [
        { 
          title: "1. Normative Requirements & Invariants", 
          content: `**R-1 (Token surface)**: SRWA MUST expose a SEP-41-compatible fungible interface:
...
**R-2 (Compliance gate)**: Every state mutation that moves balances MUST call Compliance.can_transfer(ctx: TransferCtx) -> bool and revert on false

**R-3 (Determinism)**: For a given ledger state (registries/modules/settings), can_transfer() MUST be deterministic and side-effect free

**R-4 (Identity model)**: Identity & claims align with ERC-3643 concepts:
...
\`\`\`` 
        ...
        { 
          title: "3. Function Contracts (APIs)", 
          content: `**SRWA (SEP-41 + admin + compliance hook)**
...
- freeze(addr: Address, on: bool) [admin]` 
        ...
        { 
          title: "4. Compliance Core & Modules", 
          content: `**Compliance Flow:**
...
\`\`\`` 
        ...
        { 
          title: "6. Events & Telemetry Schema", 
          content: `**Key Events:**
...
- Event integrity (missing spans)` 
    ...
    {
      id: "part3-integration",
      title: "Part III: Integration Guides",
      icon: Info,
      articles: [
        { 
...
3. **Configure Compliance**
   ...
   \`\`\`` 
        ...
        { 
          ...
          content: `**Money Market Integration:**
...
2. **Configure Oracle Adapter**
   ...
   \`\`\`` 
        ...
        { 
          ...
          content: `**AMM and Routing Integration:**
...
2. **NAV Integration**
   ...
   \`\`\`` 
    ...
    {
      id: "part4-operations",
      title: "Part IV: Operations & Governance",
      icon: AlertTriangle,
      articles: [
        { 
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto max-w-6xl px-6 py-8 space-y-8">
        ...
        <div className="space-y-2 animate-fade-in">
          <h1 className="text-h1 font-semibold text-fg-primary">Technical Documentation</h1>
          <p className="text-body-1 text-fg-secondary">
            Comprehensive technical documentation for the SRWA standard, PanoramaBlock architecture, and integration guides for institutional RWA lending on Stellar.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in">
          <Card className="card-institutional hover-lift cursor-pointer">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-body-1 font-medium text-fg-primary">Technical Vision</h3>
                <p className="text-body-2 text-fg-muted">SRWA architecture & strategy</p>
              </div>
            </div>
          </Card>
          ...
          <Card className="card-institutional hover-lift cursor-pointer">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-body-1 font-medium text-fg-primary">SRWA Specification</h3>
                <p className="text-body-2 text-fg-muted">Token & compliance standards</p>
              </div>
            </div>
          </Card>
          ...
          <Card className="card-institutional hover-lift cursor-pointer">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-body-1 font-medium text-fg-primary">Integration Guides</h3>
                <p className="text-body-2 text-fg-muted">Blend, SoroSwap, Reflector</p>
              </div>
            </div>
          </Card>
          ...
          <Card className="card-institutional hover-lift cursor-pointer">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-body-1 font-medium text-fg-primary">Operations</h3>
                <p className="text-body-2 text-fg-muted">Deployment & governance</p>
              </div>
            </div>
          </Card>
        </div>
